...
## 목적
공개 방 방문, 타인 고양이 쓰다듬기·상호작용, 방문 보상은 정식 기능이다. 다른 사용자 집은 **방문자 read-only 권한**으로 보여준다. 공개 상태·방문 기록·보상 멱등성·일일 제한을 지원하는 신규 ERD가 작성될 예정이며 구체 횟수와 보상 수치는 `TBD`다.

## 흐름
```text
대상 user_id 선택
→ 공개 여부와 방문 권한 확인
→ 방문용 read-only DTO로 USERS 배경 조회
→ PLACED_OBJECTS 조회
→ ITEMS JOIN
→ 공개 가능한 USER_CATS 조회
→ 화면 렌더링
...
방문자가 위 API를 위조하면 인증 사용자와 자원 소유자를 비교해 403으로 거절한다. 방문자 요청은 소유자 편집 API와 권한을 분리한다.

## Worst Case

### 쓰다듬기/보상 버튼 매크로 연타
- 원인: 더블클릭, 응답 지연, 자동화 요청.
- 서버 감지: 방문자·대상 방/고양이·상호작용·정책 기간으로 만든 멱등 식별자와 일일 제한 확인.
- 방어: 같은 이벤트는 기존 결과를 반환하고 재지급하지 않는다. rate limit은 멱등성을 대신하지 않는다.
- UI: 처리 중 잠금 후 서버 결과를 표시하며 보상 애니메이션을 반복하지 않는다.

### 보상 commit 후 네트워크 단절/앱 크래시
- 재접속 시 방문 기록과 지급 결과를 조회해 같은 결과와 최신 잔액을 반환한다.
- UI는 즉시 실패로 단정하지 않고 `결과 확인 중` 뒤 확정 상태를 복원한다.

### 방문자가 배치 데이터 폭탄 전송
- 방문 API는 배치 JSON을 받지 않고 최소 상호작용 식별자만 받는다.
- payload 크기·타입·허용 필드·요청 빈도를 검증하고 부분 저장하지 않는다.

## 현재 ERD 한계
하우스 공개 상태, 방문/상호작용 이력, 보상 멱등성·일일 제한 구조가 없다. 신규 ERD에서 지원해야 하며 테이블/컬럼명은 확정 전 `TBD`다.
...
9. 공개 방 노출·탐색 세부 정책
10. 방문 상호작용 횟수·보상 수치와 신규 ERD 세부 구조
